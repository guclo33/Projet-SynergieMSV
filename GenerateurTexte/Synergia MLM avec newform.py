...
def generate_simple_word(file_path, text_content):
    ...
    try:
        ...
        print(f"✅ Fichier Word créé avec succès : {unique_file_path}")

    except Exception as e:
        print(f"⚠️ Erreur lors de la génération du fichier Word : {e}")
        return None  # Retourne None pour signaler l'échec sans arrêter le script

# 📂 Création des dossiers avec gestion d'erreur
def create_directory(directory_path):
    """Crée un dossier en gérant les erreurs."""
    try:
        if not os.path.exists(directory_path):
            os.makedirs(directory_path)
            print(f"📁 Dossier créé : {directory_path}")
    except Exception as e:
        print(f"⚠️ Impossible de créer le dossier {directory_path} : {e}")

# Définition des chemins des dossiers

nouveau_dossier = f"C:/Users/Guillaume Cloutier/OneDrive/Synergia/{group_name}/profils"
dossier_facture = f"C:/Users/Guillaume Cloutier/OneDrive/Synergia/{group_name}/factures"
dossier_questionnaires = f"C:/Users/Guillaume Cloutier/OneDrive/Synergia/{group_name}/questionnaires"

# 🔹 Création des dossiers de manière sécurisée
create_directory(nouveau_dossier)
create_directory(dossier_facture)
create_directory(dossier_questionnaires)

# 🔹 Génération du fichier Word avec gestion d'erreur
try:
    ...
    print(f"⚠️ Une erreur inattendue s'est produite : {e}")
...
try:
    ...
    profil_generator_data.add_data_to_db(client_id, bref_text, forces_text, defis_text, changements_text, interpersonnelles_text, structure_text, problemes_text, arch1_nom, arch2_nom, desc_arch1_text, desc_arch2_text, travail_text, adapte_rouge_text, adapte_bleu_text, adapte_vert_text, adapte_jaune_text, bleu, rouge, jaune, vert, explorateur, protecteur, bouffon, souverain, magicien, createur, hero, citoyen, sage, amoureuse, rebelle, optimiste, motivation_text)
